...
// WO-13d (ADR 0026): direct panel modulation. DCO_LFO_DEPTH applies the shared
// LFO1 directly to DCO pitch (vibrato), independent of the mod matrix.
// PWM_MODE selects how OSC_PWM is interpreted: 0=LFO (OSC_PWM is a modulation
// amount around the hardware-neutral 50% center), 1=Manual (OSC_PWM is the
// fixed pulse width). Default Manual, matching the prior static-PWM behavior.
static constexpr uint16_t DCO_LFO_DEPTH = 0x18;  // LFO1 -> DCO pitch depth [0,1]
static constexpr uint16_t PWM_MODE      = 0x19;  // 0=LFO, 1=Manual (default)
