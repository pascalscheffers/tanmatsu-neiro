...
void test_matrix_depth_moves_dest() {
    ...
    ModMatrix mat;
    mat.clear();
    mat.set_route(0, Routing{(uint8_t)ModSource::ENV2, ParamId::FILTER_CUTOFF, 0.5f, (uint8_t)ModCurve::LIN});
    ...
    TEST_ASSERT(err < 1e-6f, "cutoff_mod with env2=1.0 and depth=0.5 must equal 0.5");
    test_pass();
}
...
void test_matrix_zero_depth_no_effect() {
    ...
    ModMatrix mat;
    mat.clear();
    mat.set_route(0, Routing{(uint8_t)ModSource::LFO1, ParamId::FILTER_CUTOFF,
    ...
    TEST_ASSERT(fabsf(out.cutoff_mod) < 1e-15f, "depth=0 route must not contribute to cutoff_mod");
    test_pass();
}
...
void test_matrix_none_source_no_effect() {
    ...
    ModMatrix mat;
    mat.clear();
    mat.set_route(0, Routing{(uint8_t)ModSource::NONE, ParamId::FILTER_CUTOFF, 1.0f, (uint8_t)ModCurve::LIN});

    ModSources src{};
    src.lfo1       = 1.0f;  // active but source is NONE
    ModOutputs out = mat.eval(src);

    TEST_ASSERT(fabsf(out.cutoff_mod) < 1e-15f, "NONE source must not contribute to cutoff_mod");
    test_pass();
}
...
void test_matrix_multi_source_summing() {
    ...
    mat.set_route(0, Routing{(uint8_t)ModSource::LFO1, ParamId::FILTER_CUTOFF, 0.3f, (uint8_t)ModCurve::LIN});
    // Route 1: ENV2 → FILTER_CUTOFF, depth 0.4 → contribution: 1.0 × 0.4 = 0.4
    mat.set_route(1, Routing{(uint8_t)ModSource::ENV2, ParamId::FILTER_CUTOFF, 0.4f, (uint8_t)ModCurve::LIN});
    ...
    TEST_ASSERT(err < 1e-6f, "sum of two routes to cutoff_mod must be 0.3+0.4=0.7");
    test_pass();
}
...
void test_matrix_cutoff_mod_in_voice() {
    ...
    vb.mod_matrix().set_route(0, Routing{(uint8_t)ModSource::ENV2, ParamId::FILTER_CUTOFF,
    ...
    TEST_ASSERT(rms_b > rms_a * 1.1f, "ENV2->cutoff routing must increase voice RMS vs no routing");
    test_pass();
}
...
void test_matrix_pitch_mod() {
    ...
    mat.set_route(0, Routing{(uint8_t)ModSource::LFO1, kModDestPitch, 12.0f, (uint8_t)ModCurve::LIN});
    ...
    TEST_ASSERT(err < 1e-6f, "LFO1=1.0 with depth=12 must give pitch_semi=12 (one octave)");
    test_pass();
}
...
void test_matrix_curve_shaping() {
    ...
    // LIN: 0.5 * 1.0 = 0.5
    mat.set_route(0, Routing{(uint8_t)ModSource::ENV2, ParamId::FILTER_CUTOFF, 1.0f, (uint8_t)ModCurve::LIN});
    float lin_out = mat.eval(src).cutoff_mod;

    // SQR: signed square of (0.5*1.0)=0.5 → 0.5*|0.5|=0.25
    mat.set_route(0, Routing{(uint8_t)ModSource::ENV2, ParamId::FILTER_CUTOFF, 1.0f, (uint8_t)ModCurve::SQR});
    float sqr_out = mat.eval(src).cutoff_mod;

    // CUBE: (0.5)^3 = 0.125
    mat.set_route(0, Routing{(uint8_t)ModSource::ENV2, ParamId::FILTER_CUTOFF, 1.0f, (uint8_t)ModCurve::CUBE});
...
void test_matrix_full_16_slots() {
    ...
    for (int i = 0; i < kMaxRoutes; i++) {
        mat.set_route(i, Routing{(uint8_t)ModSource::LFO1, ParamId::FILTER_CUTOFF, 0.1f, (uint8_t)ModCurve::LIN});
    }
    ...
    float err      = fabsf(out.cutoff_mod - expected);
    TEST_ASSERT(err < 1e-5f, "16 active routes depth=0.1 must sum to 1.6 in cutoff_mod");
    test_pass();
}
...
void test_matrix_env2_to_cutoff_math() {
    ...
    ModMatrix mat;
    mat.clear();
    mat.set_route(0, Routing{(uint8_t)ModSource::ENV2, ParamId::FILTER_CUTOFF, 0.8f, (uint8_t)ModCurve::LIN});
    ...
    float err      = fabsf(out.cutoff_mod - expected);
    TEST_ASSERT(err < 1e-6f, "ENV2=0.75, depth=0.8 must give cutoff_mod=0.6 (±1e-6)");
    test_pass();
}
...
void test_matrix_velocity_source() {
    ...
    ModMatrix mat;
    mat.clear();
    mat.set_route(0, Routing{(uint8_t)ModSource::VELOCITY,
    ...
    TEST_ASSERT(err < 1e-6f, "velocity=1.0, depth=1 must give amp_mod=1.0");
    test_pass();
}
...
void test_matrix_key_track_pitch() {
    ...
    mat.set_route(0, Routing{(uint8_t)ModSource::KEY_TRACK, kModDestPitch, 12.0f, (uint8_t)ModCurve::LIN});
    ...
    TEST_ASSERT(out_high.pitch_semi > 0.0f, "KEY_TRACK above A4 must produce positive pitch_semi");
    TEST_ASSERT(out_low.pitch_semi < 0.0f, "KEY_TRACK below A4 must produce negative pitch_semi");
    // High: 1.0 * 12 = 12; Low: -1.0 * 12 = -12
    TEST_ASSERT(fabsf(out_high.pitch_semi - 12.0f) < 1e-5f, "A5 key_track with depth=12 must give pitch_semi≈12");
    TEST_ASSERT(fabsf(out_low.pitch_semi + 12.0f) < 1e-5f, "A3 key_track with depth=12 must give pitch_semi≈-12");
    test_pass();
}
