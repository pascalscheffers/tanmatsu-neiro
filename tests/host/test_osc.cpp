...
static void check_pitch(float freq_hz) {
    char label[80];
    snprintf(label, sizeof(label), "PolyBLEP saw @ %.0f Hz: alias < −40 dB", (double)freq_hz);
    test_begin(label);
    ...
    for (int i = 0; i < kRenderFrames; i++) buf[i] = osc.Process();
    ...
    TEST_ASSERT_LT(ratio1, kAliasCeiling, "alias1 amplitude ratio exceeds -40 dB floor");
    TEST_ASSERT_LT(ratio2, kAliasCeiling, "alias2 amplitude ratio exceeds -40 dB floor");
