...
| 6a.1 rotated-panel band fix | ≈0x1128c0 (46% partition free) | — | dirty band word now `std::atomic<uint32_t>` (`ui_dirty.cpp`, no size change) | **+1.15 MB** (two 240×800×3 B = 576 KB scratch buffers, `heap_caps_malloc(MALLOC_CAP_SPIRAM)` once at display init) | present-side only | ADR 0023: 6a's band was a raw *row* range but the panel is portrait-native under 270° PAX rotation, so a logical scanline band is a raw *column* range — device now maps band→columns and packs the column window into an alternating scratch buffer before `bsp_display_blit`; bands ≥ 240 logical px (half the panel width, the pack/full-blit traffic breakeven) fall back to a full zero-copy blit. On-device PSRAM/block-time A/B still pending (Pascal) |
| 11 SD recording | `0x121650` app / 43% partition free; 1,054,762 B mapped flash | 113,674 B | **241,426 B DIRAM total** (111,052 B `.bss`, 16,700 B `.data`; includes fixed ~64 KiB record ring) | framebuffer/scratch unchanged | device PROFILE delta pending | ADR 0024: stereo 48 kHz PCM16 capture publishes once per render block into a fixed audio→control SPSC ring; SD/FATFS writing, header checkpoints, UI state, and errors remain on the control thread. PCM begins at byte 4096 behind an in-place RIFF `JUNK` header built in the existing 32 KiB staging allocation, so alignment adds no buffer. Normal `make size`, 2026-07-17. |
