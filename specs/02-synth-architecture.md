...
| 11 SD recording | `0x120830` app / 44% partition free; 1,051,150 B mapped flash | 113,674 B | **240,878 B DIRAM total** (110,504 B `.bss`, 16,700 B `.data`; includes fixed ~64 KiB record ring) | framebuffer/scratch unchanged | device PROFILE delta pending | ADR 0024: stereo 48 kHz PCM16 capture publishes once per render block into a fixed audio→control SPSC ring; SD/FATFS writing, header checkpoints, UI state, and errors remain on the control thread. Normal `make size`, 2026-07-17. |
