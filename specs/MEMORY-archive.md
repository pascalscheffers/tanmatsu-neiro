...
Older entries rotated out of [`MEMORY.md`](MEMORY.md) to keep the live log lean. History only;
the live file holds the current state + open gates. Roughly chronological, but a few early
entries were appended out of order — trust the dates, not the position.
