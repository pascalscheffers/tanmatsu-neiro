...
static const char* group_name(uint8_t g) {
    switch (g) {
        ...
        case GROUP_FX:     return "FX";
        case GROUP_AMP:    return "AMP";
        default:           return "?";
    }
}
...
extern "C" void ui_state_init(UIState* s) {
    ...
    for (int i = 0; i < kJunoParamCount; i++) {
        ...
        for (int j = 0; j < s->num_pages; j++) {
            if (s->page_groups[j] == g) { found = true; break; }
        }
...
extern "C" bool ui_handle_event(UIState* s, const platform_event_t* ev) {
    ...
    int n = (s->num_pages > 0)
            ? group_params(s->page_groups[s->page], rows, 16)
            : 0;
...
static void draw_tabs(pax_buf_t* fb, const UIState* s) {
    ...
    for (int i = 0; i < s->num_pages; i++) {
        bool sel = (i == s->page);
        pax_simple_rect(fb, sel ? COL_ACCENT : COL_TAB_DIM,
                        (float)i * tab_w, 0.0f, tab_w - 1.0f, TAB_H);
        pax_draw_text(fb, sel ? COL_BG : COL_DIM,
                      pax_font_sky_mono, FONT_MD,
                      (float)i * tab_w + 10.0f, (TAB_H - FONT_MD) * 0.5f,
                      group_name(s->page_groups[i]));
    }
}
...
static void draw_rows(pax_buf_t* fb, const UIState* s) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Selection arrow.
        if (sel) {
            pax_draw_text(fb, COL_ACCENT, pax_font_sky_mono, FONT_MD,
                          8.0f, mid_y - FONT_MD * 0.5f, ">");
        }

        // Param name.
        pax_draw_text(fb, sel ? COL_TEXT : COL_DIM,
                      pax_font_sky_mono, FONT_MD,
                      NAME_X, mid_y - FONT_MD * 0.5f, d->name);
        ...
        if (filled > 0.5f) {
            pax_simple_rect(fb, sel ? COL_ACCENT : COL_DIM,
        ...
        pax_draw_text(fb, sel ? COL_TEXT : COL_DIM,
                      pax_font_sky_mono, FONT_MD,
                      VAL_X, mid_y - FONT_MD * 0.5f, val_buf);

        // Unit label.
        const char* unit_str = nullptr;
        switch (d->unit) {
            case UNIT_HZ:   unit_str = "Hz";  break;
            case UNIT_PCT:  unit_str = "%";   break;
            case UNIT_DB:   unit_str = "dB";  break;
            case UNIT_SEMI: unit_str = "st";  break;
            case UNIT_SEC:  unit_str = "s";   break;
            case UNIT_MS:   unit_str = "ms";  break;
            default: break;
        }
        if (unit_str) {
            pax_draw_text(fb, COL_DIM, pax_font_sky_mono, FONT_SM,
                          VAL_X + 80.0f, mid_y - FONT_SM * 0.5f, unit_str);
        }
    }
}
